...
//                 price: subject.price,
//                 specialties: subject.specialties ? subject.specialties.map(s => s.id) : []
//             });
//             setSelectedSpecialties(subject.specialties ? subject.specialties.map(s => s.id) : []);
//         } catch (error) {
...
//                 : formData.specialties.filter(specialtyId => specialtyId !== value);
//
...
//             setAlert({ type: 'success', message: 'Дисциплината е актуализирана успешно!' });
...
//                             </div>
//                             <a href="/admin/subjects" className="inline-flex items-center gap-1 px-4 py-3 bg-gray-500 text-white rounded-lg shadow hover:bg-gray-700 transition-colors">
...
const EditSubject = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-50 overflow-x-hidden">
            ...
            <div className="flex">
                <Sidebar user={user} />
                <div className="flex-1 ml-0 lg:ml-0 p-4 lg:p-4"> {/* Намалено разстояние от p-8 на p-4 */}
                    <div className="bg-white/90 backdrop-blur-md shadow-sm py-4 mb-4 rounded-xl border border-gray-100"> {/* Намалено разстояние от mb-6 на mb-4 */}
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-6 gap-4">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Редактиране на дисциплина</h1>
                                <p className="text-sm text-gray-500 mt-1">Променете информацията за дисциплината</p>
