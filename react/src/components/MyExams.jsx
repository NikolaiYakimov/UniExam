...
const MyExams = () => {
    ...
    const fetchMyExams = async () => {
        try {
            ...
            console.error('Failed to fetch exams', err);
            setError(err.response?.data?.message || 'Грешка при зареждане на изпитите');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleUnregister = async (examId, examDate) => {
        ...
        if (hoursDifference < 48) {
            setError('Отписването от изпита е невъзможно по-малко от 48 часа преди изпита');
            return;
        }
        ...
        try {
            ...
            setExams(exams.filter(exam => exam.id !== examId));
            setSuccess('Успешно се отписахте от изпита');
        } catch (err) {
            console.error('Failed to unregister from exam', err);
            setError(err.response?.data?.message || 'Грешка при отписване от изпита');
        }
    };
