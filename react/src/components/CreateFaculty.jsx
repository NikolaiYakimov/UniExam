...
const CreateFaculty = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post('/faculties/store', formData);
            navigate('/faculties', {
                state: { message: 'Факултетът е създаден успешно!' },
                replace: true
            });
        } catch (error) {
            console.error('Грешка при създаване на факултет:', error);
            if (error.response?.data?.message) {
                setAlert({ type: 'error', message: error.response.data.message });
            } else {
                setAlert({ type: 'error', message: 'Грешка при създаване на факултет' });
            }
        } finally {
            setLoading(false);
        }
                ...
                <div className="flex-1 p-4 lg:p-8 ml-0 lg:ml-0">
                    <div className="bg-white/90 backdrop-blur-md shadow-sm py-4 mb-6 rounded-xl border border-gray-100">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-6 gap-4">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Добавяне на нов факултет</h1>
                                <p className="text-sm text-gray-500 mt-1">Попълнете формата за добавяне на нов факултет</p>
                            </div>
                            <Link
                                to="/faculties"
